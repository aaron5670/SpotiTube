...
public class PlaylistController {
    ...
    public Response getAllPlaylists(@Context UriInfo QueryParam) {
        ...
        if (playlists == null) return Response.status(404).build();
    ...
    @Path("/playlists")
    @Produces(MediaType.APPLICATION_JSON)
    public Response deleteAPlaylist(PlaylistDTO playlistDTO, @Context UriInfo QueryParam) {
        String token = QueryParam.getQueryParameters().getFirst("token");
        PlaylistsDTO playlists = iPlaylistDAO.addAPlaylist(playlistDTO, token);
    ...
    @Path("/playlists/{id}")
    @Produces(MediaType.APPLICATION_JSON)
    public Response editAPlaylist(PlaylistDTO playlistDTO, @Context UriInfo QueryParam) {
        String token = QueryParam.getQueryParameters().getFirst("token");
        PlaylistsDTO playlists = iPlaylistDAO.editAPlaylist(playlistDTO, token);
